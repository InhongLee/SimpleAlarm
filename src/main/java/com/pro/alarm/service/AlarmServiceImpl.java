...
public class AlarmServiceImpl extends Utlz implements AlarmService{
	...
	public void srchJobSch(Model model) throws Exception {
		...
		if(!Utlz.isNull(req.getParameter("CUST_ID"		))) daoMap.put("CUST_ID"		, req.getParameter("appUser"		));
		...
		if(!Utlz.isNull(req.getParameter("ORDERBY_TYPE"	))
		|| !Utlz.isNull(req.getParameter("ORDERBY_COL"))) {
	...
	public void updJobSch(Model model) throws Exception {
		Map<String, Object> map = model.asMap();
		HttpServletRequest req = (HttpServletRequest) map.get("req");
		if(Utlz.isNull(req.getParameter("DEVER_ID"	))) throw new AlarmException("개발자ID는 필수조건입니다."	);
		if(Utlz.isNull(req.getParameter("CUST_ID"	))) throw new AlarmException("고객ID는 필수조건입니다."	);
		if(Utlz.isNull(req.getParameter("SQNO"		))) throw new AlarmException("일련번호는 필수조건입니다."	);
		if(Utlz.isNull(req.getParameter("LUPD_CNT"	))) throw new AlarmException("최종수정수는 필수조건입니다."	);
